...
class RazonAltaChequera(Razon):
    def __init__(self, operacion, tarjetas_actuales, chequeras_actuales, monto, cupoDiarioRestante, saldoEnCuenta, cliente, motivo):
        super().__init__(operacion, tarjetas_actuales, chequeras_actuales,
                         monto, cupoDiarioRestante, saldoEnCuenta, cliente, motivo)
        self.operacion == "ALTA_CHEQUERA"
        self.motivo = self.cliente.puede_crear_chequera(
            self.chequeras_actuales)[0]
...
class RazonAltaTarjetaCredito(Razon):
    def __init__(self, operacion, tarjetas_actuales, chequeras_actuales, monto, cupoDiarioRestante, saldoEnCuenta, cliente, motivo):
        super().__init__(operacion, tarjetas_actuales, chequeras_actuales,
                         monto, cupoDiarioRestante, saldoEnCuenta, cliente, motivo)
        self.operacion == "ALTA_TARJETA_CREDITO"
        self.motivo = self.cliente.puede_crear_tarjeta_credito(self.tarjetas_actuales)[
            0]
...
class RazonRetiroEfectivo(Razon):
    def __init__(self, operacion, tarjetas_actuales, chequeras_actuales, monto, cupoDiarioRestante, saldoEnCuenta, cliente, motivo):
        super().__init__(operacion, tarjetas_actuales, chequeras_actuales,
                         monto, cupoDiarioRestante, saldoEnCuenta, cliente, motivo)
        self.operacion = "RETIRO_EFECTIVO_CAJERO_AUTOMATICO"
...
class RazonTransferenciaEnviada(Razon):
    def __init__(self, operacion, tarjetas_actuales, chequeras_actuales, monto, cupoDiarioRestante, saldoEnCuenta, cliente, motivo):
        super().__init__(operacion, tarjetas_actuales, chequeras_actuales,
                         monto, cupoDiarioRestante, saldoEnCuenta, cliente, motivo)
        self.operacion == "TRANSFERENCIA_ENVIADA"
...
class RazonTransferenciaRecibida(Razon):
    def __init__(self, operacion, tarjetas_actuales, chequeras_actuales, monto, cupoDiarioRestante, saldoEnCuenta, cliente, motivo):
        super().__init__(operacion, tarjetas_actuales, chequeras_actuales,
                         monto, cupoDiarioRestante, saldoEnCuenta, cliente, motivo)
        self.operacion == "TRANSFERENCIA_RECIBIDA"
...
class Resolver():
    ...
    def __init__(self, datos_tps):
        self.cliente = Cliente(datos_tps["nombre"], datos_tps["apellido"],
                               datos_tps["numero"], datos_tps["dni"], datos_tps["tipo"])
        self.transacciones = datos_tps["transacciones"]
        self.direccion = Direccion(calle=datos_tps["direccion"]["calle"], numeroCalle=datos_tps["direccion"]
                                   ["numero"], ciudad=datos_tps["direccion"]["ciudad"], estado=datos_tps["direccion"]["provincia"])
        ...
        for t in self.transacciones:
            if t["estado"] == "ACEPTADA":
                print(
                    f"{t['fecha']}: {t['tipo']} - {t['estado']}. Monto: ${t['monto']}\n")
            else:
